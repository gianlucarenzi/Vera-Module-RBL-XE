...
The VERA RBL-XE module requires a defined power-on boot sequence: the
Atari computer and the VERA graphics chip must remain in reset until all
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{VERA ready} (\sig{VERA\_CDONE} goes high,
          $\approx$200--500\,ms later): \sig{ATARI\_RESET\#} is released
          and the Atari boots normally.
\end{enumerate}
...
\sig{ATARI\_RESET\#} & \sig{FPGA\_CDONE}~AND~\sig{VERA\_CDONE} \\
\bottomrule
\end{tabular}
\end{center}
...
\toprule
\textbf{FPGA\_CDONE} & \textbf{VERA\_CDONE} &
\textbf{VERA\_RST\#} & \textbf{ATARI\_RST\#} & \textbf{Phase} \\
\midrule
...
\sig{ATARI\_RESET\#}.  \sig{VERA\_RESET\#} connects directly from
...
driver (e.g.\ the Atari RESET button, which pulls the line to GND via
an open-collector circuit) conflicts with the 74LVC output.

\textbf{Why 74LVC and not 74HC?}  The 74LVC family has \emph{5\,V
tolerant inputs} while operating at 3.3\,V VCC\@.  The CDONE signals
are at 3.3\,V; the Atari RESET line belongs to a 5\,V TTL system.  The
74LVC output at 3.3\,V is correctly recognised as HIGH by 5\,V TTL
...
R4  & 33\,$\Omega$   & 0402     & Series resistor on ATARI\_RESET\# \\
\bottomrule
\end{tabular}
\end{center}
...
\file{C6502\_Interface.vhd} &
    Top-level VHDL entity (Rev~4).  Generic \cmd{VERA\_IS\_PBI}
