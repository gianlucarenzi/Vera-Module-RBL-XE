...
On success, Yosys prints a resource utilisation summary.  Measured values
after place-and-route with nextpnr-ice40:
...
\midrule
ICESTORM\_LC (total)     &  87 & 5280 &  1\% \\
\quad of which LUT4 only &  52 &      &     \\
\quad of which LUT4+DFF  &   5 &      &     \\
\quad of which DFF only  &   5 &      &     \\
\quad of which CARRY only&  21 &      &     \\
\quad carry-chain legal. &   3 &      &     \\
\midrule
ICESTORM\_RAM (EBR)      &   5 &   30 & 16\% \\
SB\_IO (I/O buffers)     &  39 &   39 & 100\% \\
ICESTORM\_SPRAM          &   0 &    4 &  0\% \\
\midrule
\textbf{Fmax (CLK)}      & \multicolumn{3}{l}{\textbf{62.72\,MHz} (PASS at 25\,MHz)} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{I/O budget note:} the iCE40UP5K-SG48 exposes exactly 39 user I/O
pins.  This design uses all 39.  For this reason \sig{DIP\_SEL} is
2\,bits (not 3), allowing VERA\_CS bit selection from D[0]--D[3].
Adding a third DIP switch bit would require removing one other signal.
...
SPI flash.  Measured size: \textbf{104\,090 bytes} ($\approx$101.6\,KB).
...
\sig{DIP\_SEL[0]}   & In    & 27* & LVCMOS33 &
    DIP switch bit 0. Together with bit~1 selects which of D[0]--D[3]
    controls the VERA\_CS latch ($2^2 = 4$ choices). \\
\sig{DIP\_SEL[1]}   & In    & 28* & LVCMOS33 &
    DIP switch bit 1. See DIP\_SEL[0]. \\[2pt]
